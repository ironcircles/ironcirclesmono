...
class KeychainBackup extends StatefulWidget {
  final UserFurnace userFurnace;
  KeychainBackup(this.userFurnace);

  _KeychainBackupState createState() => _KeychainBackupState();
}

class _KeychainBackupState extends State<KeychainBackup> {
  bool _copied = false;
  String _passcode = '';
  ...
  void _continue() async {
    if (_copied == true) {
      //store the backup key in securestorage
     // SecureStorageService.writeKey(
         // KeyType.USER_KEYCHAIN_BACKUP + widget.userFurnace.userid!, _passcode);

      Navigator.of(context).pop(true);
    } else {
      FormattedSnackBar.showSnackbarWithContext(
          context, 'passcode not copied to clipboard', "", 2);
    }
  }
  ...
  Widget build(BuildContext context) {
          ...
          child: Column(
              ...
              children: <Widget>[
                ...
                Row(children: <Widget>[
                  ...
                  const Expanded(
                    child: Text("Enable Keychain backups",
                        style: TextStyle(fontSize: 18)),
                  )
                ]),
                const Padding(padding: EdgeInsets.only(bottom: 20)),
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        backgroundColor: globalState.theme.background,
        title: Text('Enable Keychain Backup',
