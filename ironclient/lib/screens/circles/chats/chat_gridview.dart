...
import 'package:ironcirclesapp/screens/circles/chats/chat_gridview_dm.dart';
import 'package:ironcirclesapp/screens/circles/chats/chat_gridview_group.dart';
import 'package:ironcirclesapp/screens/circles/chats/new.dart';
...
class ChatGridView extends StatefulWidget {
  ...
  ChatGridView({
    ...
    required this.userCircleFilter,
  ...
  State<StatefulWidget> createState() {
    return _ChatGridViewState();
  }
}

class _ChatGridViewState
    extends State<ChatGridView> /*with TickerProviderStateMixin*/ {
  ...
  List<UserCircleCache> _filtered = [];
  ...
  Widget build(BuildContext context) {
    ...
    if (_userCircles != null) {
      _filtered = [];
      _filtered.addAll(_userCircles!);

      ///First filter by Furnaces
      if (widget.furnaceFilter != all) {
        _filtered.retainWhere((userCircle) =>
            _getUserFurnace(userCircle)!.alias == widget.furnaceFilter);
      }

      ///Then filter by Circles
      if (widget.userCircleFilter != UserCircleFilterType.ALL) {
        if (widget.userCircleFilter == UserCircleFilterType.HIDDEN) {
          _filtered.retainWhere((userCircle) => userCircle.hiddenOpen == true);
        } else if (widget.userCircleFilter == UserCircleFilterType.DM) {
          _filtered.retainWhere((userCircle) => userCircle.dm == true);
        } else if (widget.userCircleFilter ==
            UserCircleFilterType.PRIVATE_VAULT) {
          _filtered.retainWhere((userCircle) =>
              userCircle.cachedCircle!.type! == CircleType.VAULT);
        } else if (widget.userCircleFilter == UserCircleFilterType.CIRCLES) {
          _filtered.removeWhere((userCircle) =>
              userCircle.cachedCircle!.type! == CircleType.VAULT ||
              userCircle.dm == true ||
              userCircle.hiddenOpen == true);
        }
      }
        ...
        : (_userFurnaces != null && _userCircles != null)
            ? DefaultTabController(
                ...
                child: Scaffold(
                        ...
                        child: TabBar(
                          ...
                          tabs: widget.showFeed
                              ? [
                                  ...
                                  _showNewCircleMessageIndicator(_filtered)
                                      ? Tab(
                                          child: Stack(
                                              alignment: Alignment.topRight,
                                              children: <Widget>[
                    ...
                    body: TabBarView(
                        children: widget.showFeed ? _withFeed() : _withoutFeed()
        ...
        children: [
          Container(
            ...
            child: FloatingActionButton(
                heroTag: null,
                onPressed: () {
                  debugPrint('pressed: ${DateTime.now()}');
                  Navigator.push(
                      context,
                      MaterialPageRoute(
                ...
                backgroundColor: globalState.theme.homeFAB, //globalState.theme.buttonIcon.withOpacity(.5),
                ),
          ),
          Padding(
        ...
        children: [
          Container(
            ...
            child: FloatingActionButton(
                ...
                backgroundColor: globalState.theme.buttonIcon.withOpacity(.2)),
          ),
        ],
      ),
    );
  ...
  Widget _buildCircles() {
    return GridView.builder(
        ...
        itemBuilder: (BuildContext context, int index) {
          UserCircleCache userCircleCache = _filtered[index];

          UserFurnace? userFurnace = _getUserFurnace(userCircleCache);

          if (userFurnace == null)
            return Container(child: Center(child: Text("pull down refresh")));
          ...
          if (userCircleCache.dm) {
            ...
            return ChatGridViewGroup(index, userFurnace, _userCircleBloc,
                userCircleCache, _goInside);
          }
        });
  }
