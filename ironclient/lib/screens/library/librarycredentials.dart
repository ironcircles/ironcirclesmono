...
import 'package:ironcirclesapp/blocs/updatetracker_bloc..dart';
...
import 'package:ironcirclesapp/screens/widgets/dialogpleasewait.dart';
import 'package:provider/provider.dart';

class LibraryCredentials extends StatefulWidget {
  ...
  @override
  LibraryCredentialsState createState() => LibraryCredentialsState();
}

class LibraryCredentialsState extends State<LibraryCredentials> {
  ...
  _convertCredentials() async {
    UpdateTracker updateTracker =
        await UpdateTrackerBloc.get(UpdateTrackerType.credentialUpgrade);

    if (updateTracker.value == false && _showPleaseWait == false) {
      _showPleaseWait = true;
      if (mounted) {
        DialogPleaseWait.showSpinner(
            context, "Converting Credentials", "Please wait...");
      }

      widget.libraryBloc.convertOldCredentials(widget.userFurnaces);
    } else {
      widget.libraryBloc.convertOldCredentials(widget.userFurnaces);
    }
  }
  ...
  void initState() {
    ...
    widget.libraryBloc.convertedCredentials.listen((circleobjects) {
      if (_showPleaseWait && mounted) {
        _showPleaseWait = false;
        Navigator.pop(context);
      }

      ///update _circleObjects
  ...
  Widget build(BuildContext context) {
    ...
    _circleObjects.retainWhere(
        (element) => (element.type == CircleObjectType.CIRCLECREDENTIAL));
    ...
    ListTile makeListTile(int index, CircleObject object) => ListTile(
          ...
          leading: Container(
            ...
            child: Icon(Icons.login, color: globalState.theme.cardLeadingIcon),
          ),
          title: Container(
  ...
  void _tapHandler(int index, CircleObject circleObject) async {
    if (_toggleIcons) {
      ...
      var updatedObject = await Navigator.push(
          ...
          MaterialPageRoute(
            builder: (context) => SubtypeCredential(
              userCircleBloc: UserCircleBloc(globalEventBloc: _globalEventBloc),
      ...
      if (updatedObject != null && updatedObject != circleObject) {
        if (mounted) {
          setState(() {
